...
from backend.app.schemas.order import OrderRead, OrderItemRead, OrderCreate, OrderUpdate, OrdersRead
...
async def create_order(db: AsyncSession, order: OrderCreate):
    ...
    for item in order.items:
        ...
        if price_product is None:
            return None
        db_item =OrderItem(order_id=new_order.id,product_id=item.product_id,quantity=item.quantity,price=price_product*item.quantity)
...
async def update_order(db: AsyncSession,order_id:int, order_update: OrderUpdate):
    ...
    if order_update.items:
        ...
        for item in order_update.items:
            new=OrderItem(order_id=old_order.id,product_id=item.product_id,quantity=item.quantity)
            db.add(new)
    await db.commit()
